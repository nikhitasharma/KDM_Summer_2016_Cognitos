...
public class StanfordcoreNLPproc {
    public static void main(String args[]) throws IOException{
        ...
        try {
            File outlemmafile = new File("AfterLemma.txt");
            outlemma = new BufferedWriter(new FileWriter(outlemmafile));
            File outnerfile = new File("AfterNer.txt");
            outner = new BufferedWriter(new FileWriter(outnerfile));
            File outposfile = new File("AfterPos.txt");
            outpos = new BufferedWriter(new FileWriter(outposfile));
        } catch (IOException e) {
            e.printStackTrace();
        }
        FileInputStream fstream = new FileInputStream("paper1.txt");
